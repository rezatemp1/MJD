...
extension HTTPAPIClient: APIClient {
  
  public func getCats(completion: @escaping ([Cat]?, NetworkErrorType?) -> Void) {
    
    
      guard let path = HTTPAPIClient.url(forComponent: HTTPAPIClient.catsPath) else {
      ...
      HTTPAPIClient.doRequest(request) { (json, error) in
    ...
    public func loadImage(url: URL, completion: @escaping (UIImage?, NetworkErrorType?) -> Void) {
